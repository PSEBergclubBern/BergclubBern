...
@section('content')
    <table border="0" cellspacing="0" cellpadding="10">
        @if(in_array('adressen', $allowed))
            <tr>
                <td colspan="4"><h2>Adressen</h2></td>
            </tr>
            <tr>
                <td colspan="3"><b>Mitgliederliste (Excel)</b></td>
                <td>
                    <button class="button button-primary button-download"
                            data-href="?page={{ $_GET['page'] }}&download=members.xls">Herunterladen
                    </button>
                </td>
            </tr>
            <tr>
                <td><b>Versandliste (Excel)</b></td>
                <td colspan="2"><label><input type="radio" class="download-address" name="download-address"
                                              value="addresses.xls"/> Komplett</label>&nbsp;&nbsp;&nbsp;<label><input
                                type="radio" class="download-address" name="download-address" value="shipping.xls"
                                checked/> Programm</label></td>
                <td>
                    <button id="button-address" class="button button-primary button-download"
                            data-href="?page={{ $_GET['page'] }}&download=shipping.xls">Herunterladen
                    </button>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <button class="button button-primary button-download"
                            data-href="?page={{ $_GET['page'] }}&download=contributions.xls">Herunterladen
                    </button>
                </td>
            </tr>
        @endif
        @if(in_array('touren', $allowed))
            ...
            <tr>
                ...
                <td colspan="2"><label for="download-calendar">Jahr:</label> <input type="number" id="download-calendar"
                                                                                    min="2006" max="{{ date("Y") }}"
                                                                                    value="{{ date("Y") }}"></td>
                <td>
                    <button id="button-calendar" class="button button-primary button-download"
                            data-href="?page={{ $_GET['page'] }}&download=calendar.pdf">Herunterladen
                    </button>
                </td>
            </tr>
        @endif
        @if(in_array('druck', $allowed))
            <tr>
                <td colspan="4"><h2>Druck</h2></td>
            </tr>
            <tr>
                <td><b>Pfarrblatt (Word)</b></td>
                <td colspan="2"><label for="download-pfarrblatt-from">Von:</label> <input type="text"
                                                                                          class="datepicker pfarrblatt-data"
                                                                                          id="download-pfarrblatt-from"
                                                                                          value="01.01.{{ date("Y") }}"
                                                                                          size="8"/> <label
                            for="download-pfarrblatt-to">Bis:</label> <input type="text"
                                                                             class="datepicker pfarrblatt-data"
                                                                             id="download-pfarrblatt-to"
                                                                             value="31.12.{{ date("Y") }}" size="8"/>
                </td>
                <td>
                    <button id="button-pfarrblatt" class="button button-primary button-download"
                            data-href="?page={{ $_GET['page'] }}&download=pfarrblatt.docx&from=01.01.{{ date('Y') }}&to=31.12.{{ date('Y') }}">
                        Herunterladen
                    </button>
                </td>
            </tr>
            <tr>
                <td rowspan="2" valign="top"><b>Programm (Word)</b></td>
                <td valign="bottom">Touren:</td>
                <td><label for="download-program-touren-from">Von:</label> <input type="text"
                                                                                  class="datepicker program-data"
                                                                                  id="download-program-touren-from"
                                                                                  value="{{ date('d.m.Y', strtotime($quarterTouren['from'])) }}"
                                                                                  size="8"/> <label
                            for="download-program-touren-to">Bis:</label> <input type="text"
                                                                                 class="datepicker program-data"
                                                                                 id="download-program-touren-to"
                                                                                 value="{{ date('d.m.Y', strtotime($quarterTouren['to'])) }}"
                                                                                 size="8"/><br/></td>
                <td rowspan="2">
                    <button id="button-program" class="button button-primary button-download"
                            data-href="?page={{ $_GET['page'] }}&download=program.docx&touren-from={{ date('d.m.Y', strtotime($quarterTouren['from'])) }}&touren-to={{ date('d.m.Y', strtotime($quarterTouren['to'])) }}&review-from={{ date('d.m.Y', strtotime($quarterRueckblick['from'])) }}&review-to={{ date('d.m.Y', strtotime($quarterRueckblick['to'])) }}&published={{ date('d.m.Y', strtotime($quarterRueckblick['from'])) }}">
                        Herunterladen
                    </button>
                </td>
            </tr>
            <tr>
                <td valign="bottom">Rückblick:</td>
                <td><label for="download-program-review-from">Von:</label> <input type="text"
                                                                                  class="datepicker program-data"
                                                                                  id="download-program-review-from"
                                                                                  value="{{ date('d.m.Y', strtotime($quarterRueckblick['from'])) }}"
                                                                                  size="8"/> <label
                            for="download-program-review-to">Bis:</label> <input type="text"
                                                                                 class="datepicker program-data"
                                                                                 id="download-program-review-to"
                                                                                 value="{{ date('d.m.Y', strtotime($quarterRueckblick['to'])) }}"
                                                                                 size="8"/></td>
            </tr>
        @endif
    </table>
